  ...
  if (loading) return (
    <div className="container mx-auto px-4 pt-44 lg:pt-60 pb-64" ref={loadingWrapperRef}>
      <div className="max-w-4xl mx-auto bg-white rounded-[3rem] shadow-2xl p-12 lg:p-20 text-center border border-gray-100 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-brand-gold/5 rounded-full blur-3xl -ml-32 -mb-32 animate-pulse" style={{ animationDelay: '1s' }}></div>

        <div className="relative z-10">
          <div className="relative mb-12 inline-block">
            {/* Multi-layered Loader */}
            <div className="w-32 h-32 border-4 border-gray-50 border-t-brand-blue border-r-brand-gold rounded-full animate-spin"></div>
            <div className="absolute inset-4 border-4 border-gray-50 border-b-brand-blue border-l-brand-gold rounded-full animate-spin-slow opacity-50"></div>
            <Sparkles className="absolute inset-0 m-auto text-brand-gold animate-pulse" size={40} />
          </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8 no-print">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-6 w-full lg:w-auto">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button onClick={() => window.open(`https://wa.me/55${CONTACT_INFO.whatsapp}?text=Olá! Acabei de ler meu diagnóstico da ${formData.companyName} e quero marcar uma reunião.`)} variant="primary" className="px-6 py-2 text-sm shadow-lg shadow-brand-gold/20">
            ...
            <div className="relative z-10">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 pb-8 border-b border-white/10">
                ...
                <div className="text-sm text-blue-100 flex flex-col gap-2 font-medium">
                  <span className="flex items-center gap-2 md:justify-end text-brand-gold uppercase text-[10px] font-black tracking-[0.2em] mb-1">Canais Oficiais</span>
                  <p className="flex items-center gap-2 md:justify-end opacity-80 backdrop-blur-sm bg-white/5 px-4 py-1 rounded-full"><Mail size={14} className="text-brand-gold" /> {CONTACT_INFO.email}</p>
                  <p className="flex items-center gap-2 md:justify-end opacity-80 backdrop-blur-sm bg-white/5 px-4 py-1 rounded-full"><MessageSquare size={14} className="text-brand-gold" /> {CONTACT_INFO.whatsappFormatted}</p>
                </div>
              </div>

              <div className="text-center">
                <div className="inline-flex items-center gap-2 bg-brand-gold text-white px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-[0.2em] mb-8 shadow-xl border border-white/20">
                ...
                <div className="inline-block p-1 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 px-8 py-3">
                  <p className="text-brand-gold text-xl md:text-2xl font-black mb-0.5">{formData.companyName}</p>
          ...
          <div className="p-8 md:p-20">
            <div className="prose prose-blue max-w-none prose-headings:font-heading prose-headings:font-black prose-p:text-gray-600 prose-p:text-lg prose-p:leading-relaxed">
              {reportText.split('\n').map((line, i) => {
                ...
                // Custom Rendering
                if (trimmed.startsWith('###')) return <h4 key={i} className="text-xl font-black text-brand-blue mt-10 mb-5 uppercase tracking-widest flex items-center gap-3"><span className="w-8 h-1 bg-brand-gold rounded-full"></span>{trimmed.replace(/###/g, '').trim()}</h4>;
                if (trimmed.startsWith('##')) return <h3 key={i} className="text-3xl font-black text-brand-darkBlue mt-16 mb-8 border-l-8 border-brand-gold pl-6 leading-tight uppercase">{trimmed.replace(/##/g, '').trim()}</h3>;
                ...
                if (trimmed.startsWith('-') || trimmed.startsWith('*')) return <li key={i} className="ml-5 mb-4 list-none flex items-start gap-4 text-gray-700 font-medium group"><CheckCircle2 className="text-brand-gold mt-1 shrink-0 group-hover:scale-110 transition-transform" size={20} />{trimmed.replace(/[-*]/, '').trim()}</li>;
            ...
            <div className="mt-32 pt-16 border-t-4 border-gray-50 relative">
              <div className="absolute -top-1 left-0 w-24 h-1 bg-brand-gold"></div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
                ...
                <div className="text-center bg-gray-50 p-8 rounded-3xl border border-gray-100 shadow-inner">
                  ...
                  <Button
                    ...
                    variant="primary"
                    className="px-8 py-4 text-xs shadow-2xl shadow-brand-gold/40 hover:-translate-y-1 transition-transform"
                  >
