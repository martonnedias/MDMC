...
const Services: React.FC<ServicesProps> = ({ onNavigate }) => {
  ...
  return (
    <section id="servicos" className={`${config.content?.sections?.services?.is_active === false ? 'hidden' : 'py-24 lg:py-40'} overflow - hidden relative bg - [#FAFBFF]`} >
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        */}
        <div className="relative space-y-16 lg:space-y-0">
          ...
          <div className="hidden lg:block absolute left-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-brand-gold/20 via-brand-blue/30 to-purple-500/20 -translate-x-1/2 -z-10">
            {/* Animated Progress Pulse */}
            <div className="w-full h-20 bg-brand-gold shadow-[0_0_15px_rgba(255,215,0,0.5)] absolute top-0 animate-[moveLine_5s_linear_infinite] [clip-path:polygon(0%_0%,100%_0%,100%_100%,0%_100%)]"></div>
          </div>
          ...
          <div className="flex flex-col gap-8 lg:gap-0">
            {activeServices.map((service, index) => {
              ...
              /* Compute node colors programmatically to avoid inline styles */
              const nodeGlowColorClass = isEven ? 'shadow-[0_0_40px_rgba(255,215,0,0.4)]' : 'shadow-[0_0_40px_rgba(0,123,255,0.4)]';
                ...
                <div key={index} className="relative">
                  ...
                  <div className={`flex flex-col lg:flex-row items-center justify-center lg:min-h-[400px] w-full ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
                    ...
                    <div className="w-full lg:w-[45%]">
                      ...
                      >
                        ...
                        <div className={`absolute inset-0 bg-gradient-to-br transition-opacity duration-700 opacity-0 group-hover:opacity-100 pointer-events-none
                          ${index === 0 ? 'from-yellow-50/80 to-transparent' : ''}
                          ...
                          ${index === 5 ? 'from-yellow-50/80 to-transparent' : ''}
                        ...
                        <div className={`absolute inset-0 p-[1.5px] rounded-[2.5rem] bg-gradient-to-br transition-opacity duration-700 opacity-0 group-hover:opacity-100
                          ${index === 0 ? 'from-brand-gold/40 via-yellow-200/20 to-transparent' : ''}
                          ...
                          ${index === 5 ? 'from-brand-gold/40 via-yellow-200/20 to-transparent' : ''}
                    ...
                    <div className="hidden lg:flex w-[10%] justify-center relative">
                      <div className={`w-14 h-14 rounded-full border-4 border-white shadow-[0_0_30px_rgba(0,0,0,0.1)] flex items-center justify-center transition-all duration-700 z-20 group-hover:scale-125 group-hover:${nodeGlowColorClass} ${isEven ? 'bg-brand-gold' : 'bg-brand-blue'}`}
                      >
                      ...
                      {insight && (
                        <div className={`max-w-xs group ${isEven ? 'text-left' : 'text-right'}`}>
                          <div className={`inline-flex items-center gap-2 mb-4 px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all duration-500
                            ${index < 2 ? 'bg-yellow-50 text-brand-gold border-yellow-100' : ''}
                    ...
                    {!isLast && (
                      <div className="lg:hidden flex flex-col items-center py-6">
                        <div className="w-1 h-12 bg-gradient-to-b from-brand-gold/30 to-brand-blue/30 rounded-full"></div>
          ...
          <div className="relative mt-24 lg:mt-32 flex flex-col items-center">
            ...
            <div className="hidden lg:block w-1 h-32 bg-gradient-to-b from-purple-500/20 to-brand-gold/40 opacity-50"></div>

            <div className="relative group w-full max-w-lg mx-auto">
              <div className="absolute -inset-1.5 bg-gradient-to-r from-brand-gold via-brand-blue to-purple-600 rounded-[3rem] blur-2xl opacity-20 group-hover:opacity-40 transition-all duration-1000"></div>
              ...
              <div className="relative bg-white border border-slate-100 p-10 lg:p-14 rounded-[3.5rem] text-center shadow-2xl overflow-hidden">
                <div className="absolute top-0 right-0 w-40 h-40 bg-brand-gold/5 rounded-full -mr-20 -mt-20 blur-3xl"></div>
                <div className="absolute bottom-0 left-0 w-40 h-40 bg-brand-blue/5 rounded-full -ml-20 -mb-20 blur-3xl"></div>

                <div className="relative z-10">
                  <div className="inline-flex items-center gap-2 mb-6 px-6 py-2 rounded-full bg-brand-gold/10 text-brand-gold text-[10px] font-black uppercase tracking-[0.3em] border border-brand-gold/20 shadow-sm">
                  ...
                  <h3 className="text-3xl lg:text-5xl font-heading font-black text-brand-darkBlue mb-8 tracking-tighter uppercase leading-[0.9]">
                    Objetivo <span className="text-brand-gold italic">Final</span>
                  </h3>

                  <p className="text-2xl lg:text-5xl font-black text-brand-darkBlue leading-[1.1] tracking-tight">
                    Lucratividade Máxima e <br className="hidden sm:block" /> <span className="bg-gradient-to-r from-brand-gold via-brand-blue to-purple-600 bg-clip-text text-transparent">Escala Sustentável.</span>
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-32 w-full bg-brand-darkBlue rounded-[3rem] p-10 lg:p-20 flex flex-col md:flex-row items-center gap-10 shadow-[0_50px_100px_-20px_rgba(0,10,40,0.4)] relative overflow-hidden border border-white/5">
          <div className="absolute top-0 right-0 w-80 h-80 bg-brand-gold/10 rounded-full -mr-40 -mt-40 blur-[120px]"></div>
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-brand-blue/5 rounded-full -ml-40 -mb-40 blur-[120px]"></div>

          <div className="relative">
            <div className="absolute -inset-8 bg-brand-gold/20 rounded-full blur-3xl animate-pulse"></div>
            <div className="relative bg-gradient-to-br from-brand-gold to-yellow-500 text-brand-darkBlue p-8 rounded-[2.5rem] shrink-0 shadow-2xl transform hover:rotate-6 transition-transform duration-500">
              <Lightbulb size={56} />
            </div>
          </div>
          ...
          <div className="text-center md:text-left relative z-10 flex-grow">
            ...
            <h4 className="font-heading font-black text-white text-3xl lg:text-5xl mb-6 tracking-tighter uppercase leading-[0.9]">Não escolha tarefas.<br />Inicie uma <span className="text-brand-gold italic">Jornada de Escala</span>.</h4>
          ...
          <div className="relative z-10">
            <button
              ...
              className="bg-brand-gold text-brand-darkBlue hover:bg-yellow-500 px-12 py-6 rounded-2xl font-black uppercase tracking-widest text-sm transition-all duration-500 shadow-brand-gold/30 hover:shadow-[0_0_30px_rgba(255,215,0,0.5)] hover:scale-105 active:scale-95 whitespace-nowrap group/btn border-none"
            >
