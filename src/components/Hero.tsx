      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
          ...
          <div className="flex-[1.2] text-center lg:text-left animate-fade-in-up flex flex-col items-center lg:items-start">
            <div className="inline-flex items-center gap-3 bg-white/5 backdrop-blur-md text-brand-gold px-5 py-2 rounded-full mb-10 font-black text-[10px] uppercase tracking-[0.3em] border border-white/10 shadow-xl">
              <Sparkles size={14} className="animate-pulse" /> Performance de Elite
            </div>

            <h1 className={`${fontSizeTitle} font-black leading-tight pb-4 tracking-tighter mb-8 max-w-4xl text-white`}
            >
              {(() => {
                ...
                return words.map((word, i) => {
                  ...
                  return (
                    <React.Fragment key={i}>
                      {isHighlighted ? (
                        <span className="relative inline-block text-transparent bg-clip-text bg-gradient-to-r from-brand-gold to-yellow-400 italic px-1">
                          {word}
                        </span>
                      ) : (
                        word
            ...
            <div className={`${fontSizeSubtitle} text-slate-400 mb-12 max-w-2xl leading-relaxed space-y-6 font-medium border-l-2 border-brand-gold/30 pl-6 lg:pl-8`}>
              <p className="italic">
                {subHeadline}
              </p>
            ...
            <div className="flex flex-col sm:flex-row gap-6 w-full lg:w-auto">
              <Button
                ...
                variant="primary"
                className="h-20 px-10 text-xs font-black uppercase tracking-[0.2em] transform hover:scale-105 transition-all shadow-2xl shadow-brand-gold/20 flex flex-col items-center justify-center min-w-[240px] bg-brand-gold hover:bg-yellow-500 text-brand-darkBlue border-none"
              >
            ...
            <div className="mt-16 flex flex-wrap items-center justify-center lg:justify-start gap-8 opacity-60 hover:opacity-100 transition-opacity">
              <div className="flex items-center gap-5">
                <a href={config.instagram_url} target="_blank" rel="noopener noreferrer" title="Instagram" aria-label="Acessar nosso Instagram" className="text-white hover:text-brand-gold transition-colors"><Instagram size={18} /></a>
                <a href={config.facebook_url} target="_blank" rel="noopener noreferrer" title="Facebook" aria-label="Acessar nosso Facebook" className="text-white hover:text-brand-gold transition-colors"><Facebook size={18} /></a>
                <a href={config.youtube_url} target="_blank" rel="noopener noreferrer" title="YouTube" aria-label="Acessar nosso canal no YouTube" className="text-white hover:text-brand-gold transition-colors"><Youtube size={18} /></a>
                {config.linkedin_url && <a href={config.linkedin_url} target="_blank" rel="noopener noreferrer" title="LinkedIn" aria-label="Acessar nosso LinkedIn" className="text-white hover:text-brand-gold transition-colors"><Linkedin size={18} /></a>}
              </div>
              ...
              <div className="flex items-center gap-6">
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white hover:text-brand-gold transition-colors"
                  >
          ...
          <div className="relative z-10 hidden lg:flex justify-center lg:justify-end flex-1">
            <div className="relative group p-4 bg-white/5 rounded-[3rem] border border-white/10 backdrop-blur-sm shadow-2xl">
              <div className="absolute -inset-1 bg-gradient-to-br from-brand-gold to-brand-blue opacity-20 blur-xl group-hover:opacity-30 transition-opacity rounded-[3rem]"></div>
              ...
              <div className="absolute -bottom-10 -left-10 bg-slate-900/90 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 animate-float group-hover:scale-105 transition-transform duration-700">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-brand-gold/20 flex items-center justify-center text-brand-gold"><Target size={18} /></div>
