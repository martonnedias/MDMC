          ...
          {displayPlans.map((plan, index) => (
            <div
              ...
              className={`relative rounded-2xl transition-all duration-300 flex flex-col h-full
                ${plan.highlight
                  ? 'bg-white border-2 border-brand-gold shadow-2xl lg:scale-105 z-10'
                  : 'bg-gray-50 border border-gray-200 shadow-lg hover:shadow-xl'
                }
              `}
            >
              {plan.highlight && (
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-brand-gold text-white px-4 py-1 rounded-full text-sm font-bold shadow-md flex items-center gap-1">
