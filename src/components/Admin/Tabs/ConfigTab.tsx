    ...
    return (
        <form onSubmit={onSave} className="space-y-4 animate-fade-in max-w-5xl pb-20">
                ...
                {expandedSection === 'general' && (
                    <div className="bg-white p-10 rounded-[2.5rem] border border-blue-50 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div className="space-y-6">
                            <div>
                                <label htmlFor="site_name" className={labelStyles}>Nome da Empresa</label>
                                <input id="site_name" type="text" value={config.site_name} onChange={(e) => setConfig({ ...config, site_name: e.target.value })} className={inputStyles} title="Nome Principal da Marca" />
                            </div>
                            <div>
                                <label htmlFor="site_phone" className={labelStyles}>Telefone de Contato</label>
                                <input id="site_phone" type="text" value={config.phone} onChange={(e) => setConfig({ ...config, phone: e.target.value })} className={inputStyles} placeholder="(00) 00000-0000" title="Telefone exibido no site" />
                            </div>
                            <div>
                                <label htmlFor="site_whatsapp" className={labelStyles}>WhatsApp Direct (Apenas números)</label>
                                <input id="site_whatsapp" type="text" value={config.whatsapp} onChange={(e) => setConfig({ ...config, whatsapp: e.target.value })} className={inputStyles} placeholder="55869..." title="Número internacional do WhatsApp (ex: 5586999...)" />
                            </div>
                        </div>
                        <div className="space-y-6">
                            <div>
                                <div className="mb-6">
                                    ...
                                    <div className="w-14 h-14 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-center overflow-hidden shrink-0">
                                        {config.logo_url ? <img src={config.logo_url} alt="Logo Principal" title="Prévia da logo principal" className="max-h-full" /> : <ImageIcon className="text-gray-300" aria-hidden="true" />}
                                    </div>
                                </div>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div className="w-14 h-14 bg-slate-800 rounded-2xl border border-gray-600 flex items-center justify-center overflow-hidden shrink-0">
                                            {config.logo_light_url ? <img src={config.logo_light_url} alt="Logo Negativa" title="Prévia da logo negativa" className="max-h-full" /> : <ImageIcon className="text-gray-500" aria-hidden="true" />}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div className="w-14 h-14 bg-slate-900 rounded-2xl border border-gray-700 flex items-center justify-center overflow-hidden shrink-0">
                                            {config.logo_footer_url ? <img src={config.logo_footer_url} alt="Logo Rodapé" title="Prévia da logo do rodapé" className="max-h-full" /> : <ImageIcon className="text-gray-500" aria-hidden="true" />}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="logo_height_header" className={labelStyles}>Altura Header</label>
                                        <select
                                            ...
                                            title="Tamanho da logo no cabeçalho"
                                        >
                                ...
                                <div>
                                    <label htmlFor="email_slogan" className={labelStyles}>Slogan E-mail</label>
                                    <input id="email_slogan" type="text" value={config.slogan || ''} onChange={(e) => setConfig({ ...config, slogan: e.target.value })} className={inputStyles} placeholder="Estrategistas Digitais em Escala" title="Frase de efeito nos e-mails automáticos" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {expandedSection === 'appearance' && (
                    <div className="bg-white p-10 rounded-[2.5rem] border border-blue-50 shadow-sm mb-8 space-y-10">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div>
                                <label htmlFor="primary_color" className={labelStyles}>Cor Primária (Bots / Acentos)</label>
                                <div className="flex items-center gap-4 bg-gray-50 p-2 rounded-2xl border border-gray-100">
                                    <input type="color" id="primary_color" value={config.primary_color} onChange={(e) => setConfig({ ...config, primary_color: e.target.value })} className="w-12 h-12 bg-transparent border-none cursor-pointer rounded-xl" title="Cor Primária" aria-label="Escolher cor primária" />
                                    <span className="text-xs font-black text-gray-500 font-mono tracking-tighter uppercase">{config.primary_color}</span>
                                </div>
                            </div>
                            <div>
                                <label htmlFor="secondary_color" className={labelStyles}>Cor Secundária (Chamadas)</label>
                                <div className="flex items-center gap-4 bg-gray-50 p-2 rounded-2xl border border-gray-100">
                                    <input type="color" id="secondary_color" value={config.secondary_color} onChange={(e) => setConfig({ ...config, secondary_color: e.target.value })} className="w-12 h-12 bg-transparent border-none cursor-pointer rounded-xl" title="Cor Secundária" aria-label="Escolher cor secundária" />
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    className={inputStyles}
                                    title="Arredondamento dos cards"
                                >
                                ...
                                <div key={key} className={`border rounded-[2rem] overflow-hidden transition-all ${isActive ? 'bg-white border-gray-100' : 'bg-gray-50 border-gray-100 opacity-60'}`}>
                                    ...
                                    {isActive && (
                                        <div className="p-8 bg-gray-50 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 border-t border-gray-100">
                                            ...
                                            <div>
                                                <label htmlFor="section_font" className={labelStyles}>Fonte Específica</label>
                                                <select
                                                    id="section_font"
                                            ...
                                            <div>
                                                <label htmlFor="section_size" className={labelStyles}>Tamanho Título</label>
                                                <select
                                                    id="section_size"
                                                    ...
                                                    className={inputStyles}
                                                    title={`Tamanho do título para a seção ${key}`}
                                                >
                                                    <option value="">Padrão</option>
                                            ...
                                            <div>
                                                <label htmlFor="section_redirect" className={labelStyles}>Ação do Botão</label>
                                                <select
                                                    id="section_redirect"
                                                    ...
                                                    className={inputStyles}
                                                    title={`Destino do botão para a seção ${key}`}
                                                >
            ...
            <div className="fixed bottom-8 right-8 z-[100] animate-bounce-in">
                ...
                >
                    {loading ? <RotateCcw size={24} className="animate-spin" aria-hidden="true" /> : <Save size={24} aria-hidden="true" />}
                </button>
            </div>
        </form>
    );
