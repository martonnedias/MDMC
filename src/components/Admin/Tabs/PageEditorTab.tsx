...
const PageEditorTab: React.FC<PageEditorTabProps> = ({
    ...
    return (
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-24">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-gray-100 pb-8">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-8 h-8 rounded-lg bg-brand-gold text-white flex items-center justify-center font-black text-xs">
                            {pageId.substring(0, 2).toUpperCase()}
                        </div>
                        <span className="text-[10px] font-black text-brand-gold uppercase tracking-[0.3em]">Editor de Página</span>
                    </div>
            ...
            {/* SOCIAL PROOF */}
            <div className={`border rounded-[2.5rem] overflow-hidden transition-all duration-500 ${expandedBlock === 'social' ? 'bg-white border-yellow-100 ring-4 ring-yellow-50/50' : 'bg-white border-gray-100'}`}>
                <button onClick={() => toggleBlock('social')} className="w-full flex items-center justify-between p-8 hover:bg-gray-50/50">
                    ...
                    {expandedBlock === 'social' ? <ChevronUp className="text-yellow-500" /> : <ChevronDown className="text-gray-300" />}
                </button>
            ...
            {/* FINAL CTA */}
            <div className={`border rounded-[2.5rem] overflow-hidden transition-all duration-500 ${expandedBlock === 'cta' ? 'bg-white border-brand-gold ring-4 ring-yellow-500/10' : 'bg-white border-gray-100'}`}>
                <button onClick={() => toggleBlock('cta')} className="w-full flex items-center justify-between p-8 hover:bg-gray-50/50">
                    <div className="flex items-center gap-5">
                        <div className="w-14 h-14 rounded-2xl bg-brand-gold text-white flex items-center justify-center shadow-lg shadow-yellow-200">
                            <Target size={24} />
                        </div>
                        <div className="text-left">
                    ...
                    {expandedBlock === 'cta' ? <ChevronUp className="text-brand-gold" /> : <ChevronDown className="text-gray-300" />}
                </button>
                {expandedBlock === 'cta' && (
                    <div className="p-8 pt-0 animate-fade-in">
