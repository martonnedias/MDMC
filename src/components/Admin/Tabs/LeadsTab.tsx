...
import { Users, Search, Download, Trash2, Layout, Mail, Phone, Calendar, Loader2 } from 'lucide-react';
import { adminService, LeadData, SwotBriefingData } from '@/src/services/adminService';

const LeadsTab: React.FC = () => {
    const [leads, setLeads] = useState<LeadData[]>([]);
    const [briefings, setBriefings] = useState<SwotBriefingData[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchLeadsData = async () => {
            const [leadsData, briefingsData] = await Promise.all([
            ...
            <header className="flex justify-between items-end gap-6">
                ...
                <div className="relative flex-grow max-w-md">
                    ...
                    <input
                        ...
                        aria-label="Buscar leads por nome ou email"
                        value={searchTerm}
                    ...
                    {filteredBriefings.map(briefing => (
                        <div key={briefing.id} className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-blue-50 transition-all group">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <span className="px-4 py-1.5 bg-yellow-100 text-yellow-700 text-[10px] font-black uppercase tracking-widest rounded-full">{briefing.plan || 'SWOT'}</span>
                            </div>
