                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    ...
                    <div className="xl:col-span-2 space-y-6">
                        <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                            <div>
                                <label htmlFor="post-title" className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 mb-2 block">Título do Post</label>
                                <input
                                    id="post-title"
                                    type="text"
                            ...
                            <div>
                                <label id="content-label" className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 mb-2 block">Conteúdo Editorial</label>
                                <RichTextEditor
                    ...
                    <div className="space-y-6">
                        <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                            ...
                            <div>
                                <label htmlFor="post-slug" className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 mb-2 block">Slug da URL</label>
                                <input
                                    id="post-slug"
                                    type="text"
                                    ...
                                    title="Identificador amigável para a URL"
                                />
                            </div>

                            <div>
                                <label htmlFor="post-category" className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 mb-2 block">Categoria</label>
                                <select
                                    id="post-category"
                                    value={editingPost.category}
                                    onChange={(e) => setEditingPost({ ...editingPost, category: e.target.value })}
                                    className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold"
                                    title="Selecione a categoria do post"
                                >
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                            ...
                            <div className="aspect-video bg-gray-50 rounded-2xl border-2 border-dashed border-gray-100 overflow-hidden flex items-center justify-center group relative cursor-pointer">
                                {editingPost.featured_image ? (
                                    <img src={editingPost.featured_image} alt="Destaque do post" className="w-full h-full object-cover" title="Imagem de destaque atual" />
                                ) : (
            ...
            <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-grow">
                    ...
                    <input
                        ...
                        aria-label="Buscar posts por título ou categoria"
                        value={searchTerm}
                ...
                <div className="flex bg-white p-1 rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <button onClick={() => setStatusFilter('draft')} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${statusFilter === 'draft' ? 'bg-yellow-100 text-yellow-700' : 'text-gray-400 hover:text-gray-600'}`}>Rascunhos</button>
                </div>
            </div>
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {filteredPosts.map(post => (
                            <tr key={post.id} className="hover:bg-gray-50/50 transition-colors group">
                                <td className="px-10 py-8">
                                    <div className="flex items-center gap-5">
                                        <div className="w-16 h-12 bg-gray-100 rounded-xl overflow-hidden shadow-sm shrink-0">
                                            {post.featured_image ? <img src={post.featured_image} alt={post.title} className="w-full h-full object-cover" title={post.title} /> : <ImageIcon className="w-full h-full p-3 text-gray-300" aria-hidden="true" />}
                                        </div>
                                        <div>
                                ...
                                <td className="px-10 py-8">
                                    <span className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${post.status === 'published' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-yellow-50 text-yellow-600 border border-yellow-100'}`}>
