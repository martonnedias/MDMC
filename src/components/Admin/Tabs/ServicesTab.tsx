        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-grow">
                    ...
                    <input
                        ...
                        aria-label="Buscar por nome, página ou categoria"
                        value={searchTerm}
                ...
                <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
                    ...
                    {pages.map(p => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border ${pageFilter === p ? 'bg-brand-gold text-white border-brand-gold shadow-lg shadow-yellow-100' : 'bg-white text-gray-500 border-gray-200 hover:border-yellow-400'}`}
                        >
                            {p.replace('-', ' ')}
                        </button>
                    ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filtered.map((service) => (
                    <div key={service.id} className={`group relative bg-white p-8 rounded-[3rem] border transition-all hover:shadow-2xl hover:-translate-y-1 ${service.is_highlighted ? 'border-brand-gold/30 shadow-lg shadow-yellow-50' : 'border-gray-100 shadow-sm'}`}>
                        {service.is_highlighted && (
                            <div className="absolute -top-3 left-8 px-4 py-1 bg-brand-gold text-white text-[9px] font-black uppercase tracking-widest rounded-full shadow-lg shadow-yellow-100 flex items-center gap-1">
                        ...
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                <div className="flex gap-2">
                                    {service.page && <span className="text-[8px] font-black text-white uppercase tracking-widest px-2 py-0.5 bg-brand-gold rounded-md">{service.page}</span>}
                        ...
                        <div className="flex items-center gap-3 pt-6 border-t border-gray-50 mt-auto">
                            <button
                                ...
                                title={`Editar pacote ${service.name}`}
                            >
                            ...
                            <button
                                ...
                                title={`Excluir pacote ${service.name}`}
                                aria-label={`Excluir pacote ${service.name}`}
                            >
                                <Trash2 size={16} />
                            </button>
                        </div>
