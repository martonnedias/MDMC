...
import {
    ...
    LogOut, ExternalLink, Sparkles, RotateCcw, ShieldCheck, Layout, Briefcase, FileSignature, FileCode,
    LifeBuoy, CreditCard
} from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { adminService, SiteConfig, BlogPost, ServiceData } from '../../services/adminService';
import { aiService } from '../../services/aiService';
import { DEFAULT_SITE_CONFIG, DEFAULT_SERVICES } from '../../constants/adminConstants';
...
import CmsPageEditorTab from './Tabs/CmsPageEditorTab';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onNavigate }) => {
    const [activeTab, setActiveTab] = useState<'dashboard' | 'config' | 'blog' | 'services' | 'leads' | 'pages' | 'proposals' | 'contracts' | 'contract-templates' | 'support' | 'billing'>('dashboard');
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            // First ensure default services exist in DB
            await adminService.syncDefaultServices(DEFAULT_SERVICES);

            const [configData, postsData, servicesData, statsData] = await Promise.all([
            ...
            const mergedConfig = configData ? {
                ...DEFAULT_SITE_CONFIG as SiteConfig,
                ...configData,
                theme: {
                    ...DEFAULT_SITE_CONFIG.theme,
                    ...configData.theme,
                    colors: { ...DEFAULT_SITE_CONFIG.theme?.colors, ...configData.theme?.colors }
                },
                content: {
                    ...DEFAULT_SITE_CONFIG.content,
                    ...configData.content,
                    sections: { ...DEFAULT_SITE_CONFIG.content?.sections, ...configData.content?.sections }
                }
            } : DEFAULT_SITE_CONFIG as SiteConfig;
    ...
    const handleSaveConfig = async (e: React.FormEvent) => {
        ...
        if (success) {
            setToast({ message: 'Configurações aplicadas com sucesso em todo o ecossistema.', type: 'success' });
    ...
    const handleSavePost = async (e: React.FormEvent) => {
        ...
        if (success) {
            setToast({ message: 'Artigo publicado com sucesso!', type: 'success' });
            setEditingPost(null);
            loadData();
            adminService.triggerRebuild();
        }
    ...
    const handleSaveService = async (e: React.FormEvent) => {
        ...
        if (success) {
            setToast({ message: 'Pacote de serviço atualizado com sucesso.', type: 'success' });
            setEditingService(null);
            loadData();
            adminService.triggerRebuild();
        } else {
            setToast({ message: 'Erro ao salvar. Slug duplicado ou conexão falhou.', type: 'error' });
        }
        setLoading(false);
    };
    ...
    const handleSyncDefaults = async () => {
        ...
        if (success) {
            setToast({ message: 'Cards estratégicos sincronizados com sucesso!', type: 'success' });
            loadData();
        } else {
            setToast({ message: 'Houve um erro na sincronização.', type: 'error' });
        }
        setLoading(false);
    };
        ...
        <div className="min-h-screen bg-white flex font-sans">
            ...
            <aside className="w-80 bg-slate-900 border-r border-slate-800 flex flex-col sticky top-0 h-screen z-50">
                ...
                <nav className="p-6 space-y-2 flex-grow">
                    ...
                    <button
                        onClick={() => { setActiveTab('proposals'); setEditingPost(null); setEditingService(null); }}
                        className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-black transition-all ${activeTab === 'proposals' ? 'bg-brand-blue text-white shadow-2xl shadow-blue-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                    >
                        <Briefcase size={18} /> Propostas (CRM)
                    </button>
                    <button
                        onClick={() => { setActiveTab('contracts'); setEditingPost(null); setEditingService(null); }}
                        className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-black transition-all ${activeTab === 'contracts' ? 'bg-brand-blue text-white shadow-2xl shadow-blue-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                    >
                        <FileSignature size={18} /> Contratos
                    </button>
                    <button
                        onClick={() => { setActiveTab('contract-templates'); setEditingPost(null); setEditingService(null); }}
                        className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-black transition-all ${activeTab === 'contract-templates' ? 'bg-brand-blue text-white shadow-2xl shadow-blue-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                    >
                        <FileCode size={18} /> Templates de Contrato
                    </button>
                    <button
                    ...
                    <button
                        onClick={() => { setActiveTab('support'); setEditingPost(null); setEditingService(null); }}
                        className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-black transition-all ${activeTab === 'support' ? 'bg-brand-blue text-white shadow-2xl shadow-blue-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                    >
                        <LifeBuoy size={18} /> Suporte (Tickets)
                    </button>
                    <button
                        onClick={() => { setActiveTab('billing'); setEditingPost(null); setEditingService(null); }}
                        className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-black transition-all ${activeTab === 'billing' ? 'bg-brand-blue text-white shadow-2xl shadow-blue-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                    >
                        <CreditCard size={18} /> Financeiro (Stripe)
                    </button>
                </nav>
                            ...
                            {activeTab === 'pages' && config && (
                                <div className="space-y-6">
                                    {selectedPage === 'list' ? (
                                        <PagesTab
                                            config={config}
                                            onEditPage={(id) => setSelectedPage(id)}
                                        />
                                    ) : ['landing', 'sites', 'ads', 'consultancy', 'swot', 'social-media', 'gmb', 'md-converte', 'about'].includes(selectedPage) ? (
                                        <PageEditorTab
                                            pageId={selectedPage}
                                            pageName={selectedPage.toUpperCase()}
                                            config={config}
                                            setConfig={setConfig}
                                            onSave={handleSaveConfig}
                                            services={services}
