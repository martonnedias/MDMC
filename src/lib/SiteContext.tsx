...
import { DEFAULT_SITE_CONFIG } from '../constants/adminConstants';
...
const defaultContext: SiteContextType = {
    config: DEFAULT_SITE_CONFIG as SiteConfig,
    loading: true
};
...
export const SiteProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const fetchConfig = async () => {
            try {
                ...
                if (data) {
                    // Merge inteligente (DB wins, defaults fill gaps)
                    const mergedConfig: SiteConfig = {
                        ...
                        theme: {
                            ...
                            typography: {
                                ...DEFAULT_SITE_CONFIG.theme?.typography,
                        ...
                        content: {
                            ...
                            ...(data.content || {}),
                            sections: {
                                ...DEFAULT_SITE_CONFIG.content?.sections,
    ...
    useEffect(() => {
        if (config) {
            ...
            root.style.setProperty('--color-brand-gold', config.secondary_color);

            // Tema Personalizado
            if (config.theme?.colors) {
                root.style.setProperty('--bg-page', config.theme.colors.background || '#0B0F19');
                root.style.setProperty('--bg-card', config.theme.colors.card_background || '#111827');
                root.style.setProperty('--bg-header', config.theme.colors.header_background || '#0B0F19');
                root.style.setProperty('--bg-footer', config.theme.colors.footer_background || '#0A0A0A');
                root.style.setProperty('--bg-top', config.theme.colors.top_background || config.theme.colors.background || '#0B0F19');
                root.style.setProperty('--bg-mid', config.theme.colors.mid_background || config.theme.colors.background || '#0B0F19');

                root.style.setProperty('--text-primary', config.theme.colors.text_primary || '#FFFFFF');
                root.style.setProperty('--text-secondary', config.theme.colors.text_secondary || '#9CA3AF');
                root.style.setProperty('--color-title', config.theme.colors.title_color || config.theme.colors.text_primary || '#FFFFFF');
                root.style.setProperty('--color-subtitle', config.theme.colors.subtitle_color || config.theme.colors.text_secondary || '#9CA3AF');
                root.style.setProperty('--color-content', config.theme.colors.content_color || config.theme.colors.text_primary || '#FFFFFF');
                root.style.setProperty('--color-accent', config.theme.colors.accent_color || config.secondary_color || '#D4AF37');

                if (config.theme.colors.brand_blue) root.style.setProperty('--color-brand-blue', config.theme.colors.brand_blue);
                if (config.theme.colors.brand_gold) root.style.setProperty('--color-brand-gold', config.theme.colors.brand_gold);
            }
