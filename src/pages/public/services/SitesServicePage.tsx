...
import { SITES_CONTENT, SITES_PLANS, SITES_FAQ, SITES_TESTIMONIALS } from '@/src/constants';
  ...
  return (
    <div className="pt-0 pb-0 font-sans">
      ...
      <section className="relative pt-44 lg:pt-60 pb-20 lg:pb-40 bg-white overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-[40%] h-[40%] bg-brand-gold/5 rounded-full blur-[100px] -ml-[5%] -mb-[5%]"></div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
              ...
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-heading font-black leading-[0.95] tracking-tighter mb-12 max-w-4xl text-brand-darkBlue"
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-6 w-full lg:max-w-xl">
                ...
                <div className="flex-[1.2] flex flex-col gap-4">
                  <Button onClick={scrollToContact} variant="primary" className="h-16 px-8 text-xs font-black uppercase tracking-widest shadow-2xl shadow-brand-gold/20 hover:scale-105 transition-transform">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-20 items-start">
              {displayMethodology.map((m, i) => (
                <div key={i} className="flex flex-col items-center text-center gap-6 group">
                  <div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center text-brand-gold border border-white/10 group-hover:scale-110 transition-transform">
                    <m.icon size={32} />
                  </div>
                  <div>
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <SectionTitle
            ...
            title={section?.problems_title || "Seu site está <span class='text-brand-gold italic'>expulsando</span> clientes?"}
      ...
      <section className="py-24 lg:py-40 bg-slate-50 relative">
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          <SectionTitle
            ...
            title={section?.pricing_title || 'Estruturas que <span class="text-brand-gold italic">geram lucro</span>'}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-20">
            {pricingPlans.map((plan, i) => (
              <div key={i} className={`relative p-10 rounded-[3.5rem] border transition-all duration-700 flex flex-col hover:translate-y-[-10px] ${plan.highlight ? 'bg-brand-darkBlue text-white border-brand-gold shadow-2xl scale-105 z-10' : 'bg-white border-slate-100 shadow-sm'}`}>
                ...
                <p className={`text-5xl font-black mb-8 ${plan.highlight ? 'text-brand-gold' : 'text-brand-darkBlue'}`}>{plan.price}</p>
                <ul className="space-y-4 mb-10 flex-1">
                  {plan.features.map((feat: string, idx: number) => (
                    <li key={idx} className="flex items-center gap-3 text-sm font-bold">
                      <CheckIcon size={14} className={plan.highlight ? 'text-brand-gold' : 'text-green-500'} />
      ...
      <section className="py-24 lg:py-40 bg-white">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <SectionTitle
            badge="Prova social"
            title="Aprovado por <span class='text-brand-gold italic pr-1'>grandes líderes</span>"
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="bg-white rounded-[3rem] p-10 shadow-3xl">
              {formState === 'success' ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <input type="text" name="name" placeholder="Nome" value={formData.name} onChange={handleChange} required className="w-full h-14 px-6 bg-slate-50 border rounded-2xl outline-none focus:border-brand-gold" />
                  <input type="tel" name="whatsapp" placeholder="WhatsApp" value={formData.whatsapp} onChange={handleChange} required className="w-full h-14 px-6 bg-slate-50 border rounded-2xl outline-none focus:border-brand-gold" />
                  <select name="projectType" title="Tipo de Projeto" value={formData.projectType} onChange={handleChange} required className="w-full h-14 px-6 bg-slate-50 border rounded-2xl outline-none focus:border-brand-gold">
                    <option value="">Tipo de Projeto</option>
                    <option value="Landing Page">Landing Page</option>
                    <option value="Site Institucional">Site Institucional</option>
                    <option value="E-commerce">E-commerce</option>
                  </select>
                  <input type="email" name="email" placeholder="E-mail" value={formData.email} onChange={handleChange} required className="w-full h-14 px-6 bg-slate-50 border rounded-2xl outline-none focus:border-brand-gold" />
            ...
            <div className="text-white">
              <h2 className="text-4xl lg:text-6xl font-black mb-6 uppercase leading-tight">Vamos construir seu <span className="text-brand-gold italic">sucesso digital</span>?</h2>
