...
import { MAESTRO_PACKAGES } from '@/src/constants';
  ...
  return (
    <div className="font-sans antialiased text-slate-700 bg-white">
      ...
      <section className="relative pt-44 lg:pt-60 pb-24 bg-brand-darkBlue text-white overflow-hidden">
        {/* Background FX */}
        <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-brand-gold/10 rounded-full blur-[120px] pointer-events-none translate-x-1/3 -translate-y-1/3"></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="text-center lg:text-left">
              ...
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-heading font-black leading-[0.95] mb-12 tracking-tighter">
                Sua equipe está perdendo vendas <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-gold to-yellow-400 italic pr-1">todos os dias</span>.
              </h1>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-6 justify-center lg:justify-start mb-12">
                <Button onClick={scrollToContact} variant="primary" className="h-16 px-10 text-xs font-black uppercase tracking-widest shadow-xl shadow-brand-gold/20">
            ...
            <div className="relative">
              <div className="bg-white border border-white/10 rounded-[2rem] p-2 relative shadow-2xl">
                ...
                />
                <div className="absolute -bottom-6 -right-6 bg-brand-gold text-brand-darkBlue p-6 rounded-3xl shadow-xl hidden md:block animate-bounce-slow">
      ...
      <section className="py-24 lg:py-32 bg-white overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <SectionTitle
            badge="Metodologia"
            title="O treinamento que transforma vendedores em <span class='text-brand-gold italic underline decoration-brand-gold/10 underline-offset-8 pr-1'>maestros.</span>"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            ...
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-r from-brand-gold/20 to-brand-blue/20 rounded-[3rem] rotate-3 blur-2xl"></div>
              <div className="bg-slate-900 p-10 rounded-[2.5rem] relative text-white border border-white/10 shadow-2xl">
                <Quote className="text-brand-gold mb-8 opacity-50" size={64} />
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-brand-gold text-brand-darkBlue rounded-full flex items-center justify-center font-black text-xl">M</div>
                  <div>
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <SectionTitle
            ...
            title="A regência completa do <span class='text-brand-gold italic'>maestro pro</span>"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-20">
            {MAESTRO_CONTENT.modules.map((mod, i) => (
              <div key={i} className="bg-white rounded-[2.5rem] p-8 border border-slate-200 hover:border-brand-gold/30 hover:shadow-xl transition-all duration-300 group">
                <div className="mb-6">
                  <span className="inline-block px-3 py-1 bg-slate-100 text-slate-500 rounded-lg text-[10px] font-black uppercase tracking-widest mb-4 group-hover:bg-brand-gold group-hover:text-brand-darkBlue transition-colors">
                    Módulo 0{i + 1}
                  </span>
                  <h3 className="text-xl font-black text-brand-darkBlue uppercase leading-tight mb-2">{mod.title}</h3>
                  <p className="text-sm font-bold text-brand-gold">{mod.subtitle}</p>
                </div>
            ...
            {MAESTRO_CONTENT.cases.map((scase, i) => (
              <div key={i} className="bg-white/5 border border-white/10 p-10 rounded-[2.5rem] backdrop-blur-sm hover:bg-white/10 transition-colors">
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-12 h-12 bg-brand-gold rounded-full flex items-center justify-center text-brand-darkBlue font-black text-lg">
      ...
      <section id="pricing" className="py-24 lg:py-32 bg-white">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <SectionTitle
            badge="Planos"
            title="Escolha seu caminho para a <span class='text-brand-gold italic'>maestria</span>"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mt-16">
            {displayPlans.map((pkg, i) => (
              <div key={i} className={`relative p-8 rounded-[2.5rem] border flex flex-col ${pkg.highlight ? 'bg-brand-darkBlue text-white border-brand-darkBlue shadow-2xl scale-105 z-10' : 'bg-white border-slate-200 text-slate-600 hover:border-brand-gold/30'}`}>
                {pkg.highlight && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-gold text-brand-darkBlue px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">
                    {pkg.badge}
                  </div>
                )}
                <div className="mb-8 text-center">
                  ...
                  <p className={`text-3xl font-black mb-1 ${pkg.highlight ? 'text-brand-gold' : 'text-slate-800'}`}>{pkg.price}</p>
                ...
                <ul className="space-y-4 mb-8 flex-1">
                  {pkg.features.map((feat, j) => (
                    <li key={j} className="flex items-start gap-3 text-xs font-medium leading-relaxed">
                      <Check size={14} className={`shrink-0 mt-0.5 ${pkg.highlight ? 'text-brand-gold' : 'text-brand-green'}`} />
                ...
                <Button
                  ...
                  className={`w-full h-12 text-[10px] font-black uppercase tracking-widest ${!pkg.highlight && 'border-slate-200 hover:border-brand-gold hover:text-brand-gold'}`}
                >
                  {pkg.cta}
                </Button>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={handleChange}
                        required
                        title="Escolha a modalidade de consultoria"
                        className="w-full h-16 px-6 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/10 transition-all text-slate-700 font-bold cursor-pointer appearance-none"
                      >
            ...
            <div className="text-white text-left order-1 lg:order-2">
              <SectionTitle
                ...
                title={<>Pronto para transformar sua <span className="text-brand-gold italic">equipe</span>?</>}
