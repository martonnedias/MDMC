...
import { SWOT_SECTION_CONTENT, SWOT_PLANS, FORM_VALIDATION_MSGS } from '@/src/constants';
  ...
  return (
    <div className="bg-slate-50 font-sans leading-normal">
      ...
      <section className="relative pt-44 pb-20 lg:pt-60 lg:pb-32 overflow-hidden bg-brand-darkBlue text-white">
        ...
        <div className="absolute top-[-10%] right-[-5%] w-[600px] h-[600px] bg-brand-gold/10 rounded-full blur-[120px] animate-pulse pointer-events-none"></div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 text-white px-4 py-1.5 rounded-full mb-8 font-black text-[9px] uppercase tracking-[0.2em] border border-white/10 backdrop-blur-md">
            <ShieldCheck size={14} className="text-brand-gold" /> Inteligência de Negócios
          </div>
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-heading font-black mb-12 leading-[0.95] pb-2 tracking-tighter max-w-5xl mx-auto text-white">
            {section?.title || <>Pare de operar no escuro. Tenha <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-gold to-yellow-400 italic pr-1">clareza total.</span></>}
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <Button onClick={() => document.getElementById('pricing')?.scrollIntoView({ behavior: 'smooth' })} variant="primary" className="h-16 px-10 text-sm font-black uppercase tracking-widest shadow-xl shadow-brand-gold/20">
              Ver Opções de Auditoria
            </Button>
            <button onClick={scrollToContact} className="h-16 px-10 text-sm font-black uppercase tracking-widest text-white hover:text-brand-gold transition-colors flex items-center gap-2">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-yellow-50 p-8 rounded-3xl border border-yellow-100 items-start gap-4 hover:shadow-lg transition-all">
      ...
      <section className="py-24 bg-brand-darkBlue relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          <SectionTitle
            title="O método <span class='text-brand-gold italic pr-1'>s.w.o.t. profissional</span>"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-8 rounded-3xl hover:bg-white/10 transition-colors group">
              ...
              <p className="text-blue-100/90 leading-relaxed border-l-2 border-yellow-500/50 pl-4 font-medium">
            ...
            {displayPlans.map((pkg, i) => (
              <div
                ...
                className={`p-10 rounded-[3rem] border transition-all duration-500 relative flex flex-col ${pkg.highlight
                  ? 'bg-brand-darkBlue text-white border-brand-gold shadow-[0_40px_80px_-15px_rgba(255,102,0,0.15)] scale-105 z-10'
                  : 'bg-white border-slate-200 hover:border-brand-blue/30 shadow-sm hover:shadow-xl'
                  }`}
              >
                {pkg.badge && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-gold text-white px-6 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">
                    {pkg.badge}
                  </div>
                )}
                ...
                <div className="mb-10 text-center">
                  ...
                  <p className={`text-[10px] font-black uppercase tracking-[0.2em] mb-8 ${pkg.highlight ? 'text-brand-gold' : 'text-brand-blue'}`}>
                    {pkg.subtitle}
                  </p>
                ...
                <ul className="space-y-4 mb-10 flex-1">
                  {pkg.features.map((feat: string, j: number) => (
                    <li key={j} className="flex gap-4 text-sm font-medium leading-relaxed">
                      <Check size={18} className={`shrink-0 mt-0.5 ${pkg.highlight ? 'text-brand-gold' : 'text-brand-green'}`} />
                ...
                <Button
                  ...
                  fullWidth
                  className={`h-16 text-[10px] font-black uppercase tracking-widest ${!pkg.highlight && 'border-slate-200 hover:border-brand-gold hover:text-brand-gold'
                    }`}
                >
                  {pkg.cta}
                </Button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
              ...
              {formState === 'success' ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10 w-full">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={handleChange}
                        required
                        className="w-full h-16 px-6 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/10 transition-all text-slate-700 font-bold cursor-pointer appearance-none shadow-inner"
                      >
                  ...
                  <div className="pt-4 text-center">
                    <Button
                      type="submit"
                      className="w-full h-20 text-lg font-black uppercase tracking-widest bg-brand-gold hover:bg-brand-gold/90 shadow-xl shadow-brand-gold/20 rounded-2xl"
            ...
            <div className="text-white text-left order-1 lg:order-2">
              <SectionTitle
                ...
                title={<>Vamos <span className="text-brand-gold italic pr-1">mapear</span> seu futuro?</>}
