...
import { GMB_CONTENT } from '@/src/constants';
import {
...
const GoogleBusinessProfile: React.FC = () => {
  ...
  return (
    <div className="pt-0 pb-0 bg-white font-sans">
      {/* 1. HERO SECTION */}
          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
              <div className="inline-flex items-center gap-2 bg-white text-brand-blue px-4 py-1.5 rounded-full mb-10 font-black text-[9px] uppercase tracking-[0.3em] border border-slate-200 shadow-sm">
                <Sparkles size={14} className="text-brand-gold animate-pulse" /> Visibilidade Geo-Localizada
              </div>
              ...
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-heading font-black leading-[0.95] tracking-tighter mb-12 text-brand-darkBlue max-w-4xl">
                Coloque sua empresa no <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-gold to-yellow-400 italic pr-1">mapa</span> com o Google Meu Negócio.
              </h1>

              <div className="space-y-8 mb-16 border-l-4 border-brand-gold/30 pl-8">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-8 w-full lg:justify-start">
                ...
                <div className="flex items-center gap-6 opacity-60 hover:opacity-100 transition-opacity">
                  <div className="flex items-center gap-5">
                    <a href={config.instagram_url} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-brand-gold transition-colors" title="Instagram"><Instagram size={18} /></a>
                    <a href={config.facebook_url} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-brand-gold transition-colors" title="Facebook"><Facebook size={18} /></a>
                    <a href={config.youtube_url} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-brand-gold transition-colors" title="Youtube"><Youtube size={18} /></a>
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            ...
            <div className="relative group order-1 lg:order-2">
              <div className="absolute -inset-10 bg-brand-gold/5 rounded-full blur-[100px] group-hover:bg-brand-gold/10 transition-all duration-1000"></div>
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 bg-brand-gold text-brand-darkBlue rounded-full flex items-center justify-center shadow-3xl animate-float">
                ...
                <ul className="mt-auto space-y-2">
                  {pillar.features.map((f, idx) => (
                    <li key={idx} className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-2">
                      <div className="w-1 h-1 bg-brand-gold rounded-full"></div> {f}
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <section className="py-20 lg:py-32 bg-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <SectionTitle
            ...
            title={<>A jornada até o <span className="text-brand-gold italic">resultado</span></>}
      ...
      <section className="py-24 lg:py-48 bg-brand-darkBlue relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          ...
          <SectionTitle
            badge="Impacto real"
            title={<>O que você pode <span className="text-brand-gold italic">esperar</span> de nós</>}
            alignment="center"
            light={true}
          />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            <div className="text-left">
              <ul className="grid grid-cols-1 gap-6">
                {GMB_CONTENT.results.items.map((item, i) => (
                  <li key={i} className="flex items-center gap-5 group bg-white/5 p-5 rounded-2xl border border-white/5 hover:border-brand-gold/30 transition-all duration-500">
                    <div className="w-10 h-10 rounded-xl bg-brand-gold/10 flex items-center justify-center shrink-0 border border-white/10 text-brand-gold group-hover:bg-brand-gold group-hover:text-white transition-all shadow-lg">
            ...
            <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-br from-brand-gold to-brand-blue opacity-20 blur-2xl group-hover:opacity-30 transition-opacity duration-1000"></div>
              <div className="relative p-10 md:p-14 bg-white rounded-[3rem] text-slate-900 shadow-3xl border-4 border-white/5 overflow-hidden">
                <div className="bg-brand-gold text-brand-darkBlue px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest inline-block mb-8 shadow-md">Mini Case de Sucesso</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto mt-16 px-4">
            {displayPackages.map((pkg, i) => (
              <div key={i} className={`p-10 rounded-[2.5rem] border transition-all duration-700 flex flex-col items-center text-center group h-full ${pkg.highlight ? 'bg-brand-darkBlue text-white border-brand-darkBlue shadow-2xl scale-105 z-10' : 'bg-slate-50 border-slate-100 hover:bg-white hover:shadow-xl'}`}>
                {pkg.highlight && (
                  <div className="bg-brand-gold text-brand-darkBlue px-6 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.2em] mb-8 shadow-md animate-pulse">Recomendado</div>
                )}
                ...
                <div className="text-4xl font-heading font-black tracking-tighter mb-10 uppercase italic text-brand-gold border border-white/10 rounded-2xl px-5 py-2 inline-block">
                  {pkg.price}
                </div>
                ...
                <ul className="space-y-4 mb-12 flex-grow text-left w-full border-t border-white/5 pt-8">
                  {pkg.features.map((f, idx) => (
                    <li key={idx} className="flex items-start gap-3 group/li">
                      <div className={`w-6 h-6 rounded-lg ${pkg.highlight ? 'bg-brand-gold text-brand-darkBlue' : 'bg-brand-blue/10 text-brand-blue'} flex items-center justify-center shrink-0 mt-0.5 shadow-sm`}>
            ...
            {GMB_CONTENT.faqs.map((faq, i) => (
              <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 hover:border-brand-blue/10 transition-all duration-500 shadow-sm group">
                <h4 className="text-lg font-black text-brand-darkBlue mb-4 flex gap-3 tracking-tighter items-start">
                  <span className="text-brand-gold bg-brand-gold/10 px-2 py-0.5 rounded text-xs">Q.</span> {faq.question}
                </h4>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2 text-left">
                      ...
                      <select
                        ...
                        onChange={handleChange}
                        required
                        title="Informe o status atual do seu perfil"
                        className="w-full h-16 px-6 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/10 transition-all text-slate-500 font-bold cursor-pointer appearance-none shadow-inner"
                      >
            ...
            <div className="text-white text-left order-1 lg:order-2">
              <SectionTitle
                ...
                title={<>Pronto para aparecer de <span className="text-brand-gold italic">verdade</span>?</>}
