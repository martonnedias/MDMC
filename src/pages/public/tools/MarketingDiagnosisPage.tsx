...
const MarketingDiagnosisPage: React.FC<MarketingDiagnosisPageProps> = ({ onStart }) => {
  ...
  const handleNextOrSubmit = async (e: React.FormEvent) => {
    ...
    const result = await leadService.saveBriefingLead({
      ...formData,
      type: 'TESTE_GRATUITO',
      plan: 'Teste Gratuito via Site'
    });
    ...
    if (result && result.success) {
      if (result.token) {
        ...
        alert("Diagnóstico gerado com sucesso! Entraremos em contato.");
        onStart();
      }
    } else {
  ...
  return (
    <div className="pt-0 pb-12 lg:pb-24 font-sans dynamic-font" style={{ '--dynamic-font-family': section?.font_family } as React.CSSProperties}>
      ...
      <section className="pt-44 lg:pt-60 pb-12 lg:pb-32 bg-gradient-to-b from-blue-50/50 to-white overflow-hidden relative">
        ...
        <div className="absolute bottom-0 left-[-10%] w-[500px] h-[500px] bg-brand-gold/5 rounded-full blur-[100px] pointer-events-none"></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="text-center lg:text-left flex flex-col items-center lg:items-start order-2 lg:order-1">
              <div className="inline-flex items-center gap-2 bg-white text-brand-blue px-4 py-1.5 rounded-full mb-8 font-black text-[9px] uppercase tracking-[0.2em] border border-blue-100 shadow-sm animate-fade-in">
                <Sparkles size={14} className="text-brand-gold animate-pulse" /> Inteligência de Vendas
              </div>
            ...
            <div className="relative order-1 lg:order-2 w-full max-w-md mx-auto lg:mr-0">
              <div className="absolute -inset-4 bg-gradient-to-tr from-brand-blue/20 to-brand-gold/20 rounded-[3rem] blur-2xl opacity-70 animate-pulse-slow"></div>
                ...
                {user ? (
                  <div className="text-center py-6">
                    ...
                    <div className="text-left mb-6">
                      <label htmlFor="auth-user-id" className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-2 block">Usuário Identificado</label>
                  ...
                  <form onSubmit={handleNextOrSubmit} className="space-y-5">
                    {currentStep === 0 && (
                      <div className="space-y-4">
                        <div className="relative group">
                          <label htmlFor="diag-name" className="sr-only">Nome do Responsável</label>
                        ...
                        <div className="relative group">
                          <label htmlFor="diag-email" className="sr-only">E-mail Comercial</label>
                        ...
                        <div className="relative group">
                          <label htmlFor="diag-phone" className="sr-only">WhatsApp (com DDD)</label>
                    ...
                    {currentStep === 1 && (
                      <div className="space-y-4 animate-fade-in">
                        <label htmlFor="diag-company" className="sr-only">Nome da Empresa</label>
                        <input
                          id="diag-company"
                          required
                          placeholder="Nome da Empresa"
                          title="Nome da Empresa"
                          className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:border-brand-blue outline-none transition-all font-medium text-slate-700"
                          value={formData.companyName}
                          onChange={e => setFormData({ ...formData, companyName: e.target.value })}
                        />
                        <label htmlFor="diag-segment" className="sr-only">Segmento</label>
                        <select
                          id="diag-segment"
                          required
                        ...
                        >
                          ...
                          <option value="B2B">B2B (Serviços/Indústria)</option>
                          <option value="B2C">B2C (Varejo/Local)</option>
                    ...
                    {currentStep === 2 && (
                      <div className="space-y-4 animate-fade-in">
                        <label htmlFor="diag-goal" className="sr-only">Objetivo de Marketing</label>
                        <select
                          id="diag-goal"
                          required
                        ...
                        >
                          <option value="">Objetivo de Marketing</option>
                          <option value="leads">Gerar Mais Vendas (Leads)</option>
                          <option value="branding">Construir Autoridade</option>
                          <option value="reestruturacao">Reestruturação Digital</option>
                        </select>
                        <label htmlFor="diag-challenge" className="sr-only">Maior desafio atual</label>
                        <textarea
                          id="diag-challenge"
                          required
                          placeholder="Qual o seu maior desafio atual?"
                          title="Qual o seu maior desafio atual?"
                          rows={3}
                          className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:border-brand-blue outline-none transition-all font-medium text-slate-700 resize-none"
                    ...
                    <div className="flex gap-2">
                      {currentStep > 0 && (
                        <Button type="button" variant="secondary" onClick={() => setCurrentStep(prev => prev - 1)} className="flex-1">
                          Voltar
                        </Button>
                      )}
                      <Button type="submit" variant="primary" loading={loading} className="flex-[2] py-4 text-sm font-black uppercase tracking-widest rounded-xl shadow-xl shadow-brand-blue/20 hover:shadow-brand-blue/30 transition-shadow">
                        {currentStep < 2 ? 'Avançar Passo' : 'Gerar Diagnóstico'}
                      </Button>
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-16">
            {[
              ...
              { icon: Target, title: "Conversão", text: "Seu site vende ou só informa?", color: "gold" },
      ...
      <section className="py-24 bg-brand-darkBlue text-white relative overflow-hidden">
        ...
        <div className="absolute top-[-10%] right-[-5%] w-[500px] h-[500px] bg-brand-gold/10 rounded-full blur-[100px] pointer-events-none"></div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10">
          <SectionTitle
            ...
            title="Como funciona a <span class='text-brand-gold italic'>magia?</span>"
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={i} className="relative group">
                {i < 3 && (
                  <div className="hidden md:block absolute top-12 left-1/2 w-full h-[1px] bg-gradient-to-r from-white/20 to-transparent group-hover:from-brand-gold/40 transition-all duration-500"></div>
                )}
                <div className="relative bg-white/[0.03] border border-white/10 p-8 rounded-[2rem] backdrop-blur-md hover:bg-white/[0.08] hover:border-white/20 transition-all duration-300 text-center shadow-2xl">
                  <div className="w-20 h-20 mx-auto bg-gradient-to-br from-brand-darkBlue to-brand-navy border border-white/10 rounded-[1.5rem] flex items-center justify-center mb-6 shadow-xl group-hover:scale-110 group-hover:border-brand-gold/50 transition-all duration-500 relative z-10">
                    <step.icon size={28} className="text-white group-hover:text-brand-gold transition-colors" />
                    {/* Step number badge */}
                    <div className="absolute -top-2 -right-2 w-7 h-7 bg-brand-gold text-white text-[10px] font-black rounded-full flex items-center justify-center shadow-lg border-2 border-brand-darkBlue">
                      {i + 1}
                    </div>
                  </div>
                  <h4 className="text-xl font-bold mb-2 group-hover:text-brand-gold transition-colors">{step.title}</h4>
      ...
      <section className="py-24 bg-white text-center">
        <div className="max-w-4xl mx-auto px-4">
          ...
          />
          <div className="mt-10">
            <Button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} variant="primary" className="px-12 py-6 text-lg rounded-2xl shadow-xl shadow-brand-gold/20" withIcon>
